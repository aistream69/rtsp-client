...
BasicTaskSchedulerEpoll::BasicTaskSchedulerEpoll()
  ...
  if (fEpollFd == -1){
	perror("BasicTaskSchedulerEpoll epoll_create error\n");
  }
}
...
void BasicTaskSchedulerEpoll
  ...
  if (conditionSet == 0) {
    ...
    if (conditionSet&SOCKET_READABLE){
		...
		ev.events = EPOLLIN;
		 if (epoll_ctl(fEpollFd, EPOLL_CTL_ADD, socketNum, &ev) == -1){
			perror("BasicTaskSchedulerEpoll epoll_ctl SOCKET_READABLE EPOLL_CTL_ADD error \n");
		 }
	}
    if (conditionSet&SOCKET_WRITABLE){
		...
		ev.events = EPOLLOUT;
		 if (epoll_ctl(fEpollFd, EPOLL_CTL_ADD, socketNum, &ev) == -1){
			perror("BasicTaskSchedulerEpoll epoll_ctl SOCKET_WRITABLE EPOLL_CTL_ADD error\n");
		 }
	}
    if (conditionSet&SOCKET_EXCEPTION){
		...
		ev.events = EPOLLERR;
		 if (epoll_ctl(fEpollFd, EPOLL_CTL_ADD, socketNum, &ev) == -1){
			perror("BasicTaskSchedulerEpoll epoll_ctl SOCKET_EXCEPTION EPOLL_CTL_ADD  error\n");
		 }
	}
  }
}
